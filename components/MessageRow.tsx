...
const MessageRow = memo(({ index, style, data }: ListChildComponentProps<MessageRowData>) => {
      ...
      <div className="flex justify-start animate-fade-in">
        <div className={`rounded-2xl p-4 shadow-sm ${
          ...
          <div className={`mt-2 text-xs font-mono ${
            isDarkMode ? 'text-emerald-600' : 'text-emerald-400'
          } animate-pulse select-none`}>▋</div>
        </div>
      </div>
    );
    ...
    content = (
      <div className={`flex ${
        ...
        <div className={`rounded-2xl p-4 shadow-sm relative ${
          isBot
            ? `${isDarkMode ? 'bg-slate-900' : 'bg-white'} border ${
                isDarkMode ? 'border-gray-700/30' : 'border-gray-200'
              } px-3 md:px-5 py-3 md:py-4 w-full`
            : `${
                isDarkMode
