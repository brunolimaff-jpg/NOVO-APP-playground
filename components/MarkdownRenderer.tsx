...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({
  ...
  const processedContent = useMemo(() => {
    ...
    // 1) Converter {"mermaid":"..."} → bloco de código mermaid
    //    IMPORTANTE: usar concatenação de string para evitar backticks triplos
    //    dentro de template literal (bug esbuild/vite).
    const FENCE = '`'.repeat(3);
    text = text.replace(/\{"mermaid":"([\s\S]*?)"\}/g, (_match, raw) => {
      const unescaped = String(raw).replace(/\\n/g, '\n');
      return '\n' + FENCE + 'mermaid\n' + unescaped + '\n' + FENCE + '\n';
    });

    // 2) Corrigir links falsos e limpar bloco textual de fontes
    text = fixFakeLinks(text);
    text = rewriteMarkdownLinksToGoogle(text);
    text = autoLinkSeniorTerms(text);
    text = cleanFakeSourcesBlock(text);

    // 3) Badge de fonte oficial [🟢 CONFIRMADO OFICIAL - ...]
    text = text.replace(
  ...
  const components: any = {
    // code — único renderer: trata Mermaid E inline code (não duplicar!)
  ...
  // Fontes ficam APENAS no ícone de livro / painel lateral — sem duplicação aqui.
  return (
