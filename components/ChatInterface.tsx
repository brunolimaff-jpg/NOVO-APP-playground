...
const QUICK_ACTIONS = [
  { icon: "🎯", label: "Comparar Concorrentes", prompt: "Compare esta empresa com seus principais concorrentes." },
  { icon: "💰", label: "Estimar Budget", prompt: "Qual o budget estimado para implementação completa com ERP, HCM e GAtec?" },
  { icon: "💡", label: "Melhor Abordagem", prompt: "Me sugira a melhor estratégia e abordagem para os tomadores de decisão desta empresa." },
  { icon: "✨", label: "Fit Senior", prompt: "Como os produtos Senior resolveriam as dores dessa empresa?" }
];
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  onSendMessage, onFeedback, onSendFeedback, onLoadMore, onClearChat,
  onRegenerateSuggestions, onStop, onReportError, isDarkMode, onToggleTheme,
  onToggleMessageSources, exportStatus, pdfReportContent, onOpenEmailModal,
  onOpenFollowUpModal, userId, lastUserQuery, processing, onExportPDF
}) => {
                ...
                return (
                  <div key={msg.id} className={`flex ${isBot ? 'justify-start' : 'justify-end'} animate-fade-in`}>
                    <div className={`rounded-2xl p-4 shadow-sm relative group ${isBot ? `${isDarkMode ? 'bg-slate-900' : 'bg-white'} border border-gray-700/30 px-3 md:px-5 py-3 md:py-4 w-full` : `${isDarkMode ? 'bg-emerald-900/20 border border-emerald-900/30 text-emerald-100' : 'bg-emerald-50 border border-emerald-100 text-slate-800'} max-w-[90%] md:max-w-[75%] lg:max-w-[60%]`}`}>
                      ...
                      {isBot ? (
                        <>
                           {msg.scorePorta && <ScorePorta {...msg.scorePorta} />}
                           <SectionalBotMessage 
                           ...
                           {/* Aprofundar Topics mantidos originais, apenas mostrados no último bot */}
                           {isLast && !isLoading && (
                             <DeepDiveTopics 
                           ...
                           {/* Ações da Mensagem isoladas da barra de input */}
                           <MessageActionsBar 
                               content={msg.text}
