  ...
  const handleDeepDive = async (displayMessage: string, hiddenPrompt: string) => {
    ...
    if (currentSession?.empresaAlvo) {
        finalPrompt = hiddenPrompt
          .replace(/\[NOME DA EMPRESA\]/gi, currentSession.empresaAlvo)
          .replace(/\[Nome da Empresa\]/gi, currentSession.empresaAlvo)
          .replace(/\[Empresa\]/gi, currentSession.empresaAlvo)
          .replace(/\[NOME DO GRUPO \/ EMPRESA ALVO\]/gi, currentSession.empresaAlvo);
    }
